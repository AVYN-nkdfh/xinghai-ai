...
- [x] 单选时机器卡保持原有紧凑状态。
- [x] 多选时每台机器逐项显示全部所选时段状态。
- [x] “上午可用、下午已约”的机器同时显示“上午 可选”和“下午 已约”，且整台机器不可点击。
- [x] 逐时段明细完全使用当天缓存，切换和多选不新增接口请求。
- [x] 保存混合状态截图：`prototype/screenshots/booking-mixed-slot-status.png`。
- [x] 正式域名页面、脚本和接口均返回 200，线上文件哈希与本地构建一致。
- [x] v1.3 正式部署：`dpl_3qoAQ8j9cuBJBAnGb5Qkr1A9wuRf`。
